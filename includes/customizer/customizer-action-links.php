...
class Customizer_Action_Links extends \WP_Customize_Control {
		...
		switch ( $action_link_type ) {
			case 'install-elementor':
				$action_link_data = [
					...
					'title' => esc_html__( 'Install Elementor', 'hello-plus' ),
					'message' => esc_html__( 'Create cross-site header & footer using Elementor.', 'hello-plus' ),
					'button' => esc_html__( 'Install Elementor', 'hello-plus' ),
					'link' => wp_nonce_url(
						add_query_arg(
							[
			...
			case 'activate-elementor':
				$action_link_data = [
					'image' => get_template_directory_uri() . '/assets/images/elementor.svg',
					'alt' => esc_attr__( 'Elementor', 'hello-plus' ),
					'title' => esc_html__( 'Activate Elementor', 'hello-plus' ),
					'message' => esc_html__( 'Create cross-site header & footer using Elementor.', 'hello-plus' ),
			...
			case 'style-header-footer':
				$action_link_data = [
					...
					'button' => esc_html__( 'Start Designing', 'hello-plus' ),
	...
	private function get_customizer_action_links_html( $data ) {
		...
		return sprintf(
			'<div class="hello-plus-action-links">
				<img src="%1$s" alt="%2$s">
				<p class="hello-plus-action-links-title">%3$s</p>
				<p class="hello-plus-action-links-message">%4$s</p>
